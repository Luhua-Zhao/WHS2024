...
# replaicate Figure 1.1 in WHS 2024 chapter 1.
#read le_hale.csv dataset.
#le_hale<- read.csv("LE_HALE.csv")
#save(le_hale,file="LE_HALE")
#le_hale<-load ("LE_HALE")
...
plot_data <- global_trends %>%
  ...
  select(year, sex,Healthy_LE, Unhealthy_LE) %>%
...
ggplot(data = plot_data, aes(x = factor(sex), y = Value, fill = interaction(Category,sex))) +
  ...
  # Separate Male and Female bars for each year
  facet_wrap(~year, ncol = length(unique(plot_data$year)), scales = "free_x", strip.position = "bottom")
